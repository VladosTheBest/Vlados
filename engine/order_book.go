...
type OrderBook interface {
	...
	LoadFromOrders(marketID string, orders []model.Order, eventSeqID, tradeSeqID uint64) error
	GetMarket() []*SkipList
	GetMarketID() string
...
type dump struct {
	Bids        []dumpPriceLevel
	Asks        []dumpPriceLevel
	StopEntries []dumpPriceLevel
	StopLoss    []dumpPriceLevel
}
...
func NewOrderBook(marketID string, pricePrecision, volumePrecision int) OrderBook {
	return &orderBook{
		...
		// Market order data
		BuyMarketEntries:  make([]model.Order, 0),
		SellMarketEntries: make([]model.Order, 0),
		// Stop order data
...
func (book orderBook) DumpOrderbook() string {
	return ""
}
