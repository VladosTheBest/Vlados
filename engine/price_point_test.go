...
func TestPricePointCreation(t *testing.T) {
	...
	Convey("I should be able to create Price Points list and get an iterator for it", t, func() {
		...
		firstPoint := &PricePoint{Entries: []model.Order{{Amount: 1}}}
		secondPoint := &PricePoint{Entries: []model.Order{{Amount: 2}}}
		thirdPoint := &PricePoint{Entries: []model.Order{{Amount: 3}}}
		...
		Convey("Should panic at set for key=0", func() {
			So(func() {
				pricePoints := NewPricePoints()
				pricePoints.Set(0, &PricePoint{Entries: []model.Order{{Amount: 1}}})
			}, ShouldNotPanic)
		})
